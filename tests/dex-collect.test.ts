...
describe('deciding when to send on its own', () => {
  const base = { collect: true, autoSend: true, lastSentAt: null, sentTotal: 0 };
  const now = Date.parse('2026-09-20T12:00:00.000Z');

  test('a question goes as soon as it is saved', () => {
    expect(dueToSend(base, 1, true, now)).toBe(true);
  });

  test('nothing goes when nothing new was saved', () => {
    expect(dueToSend(base, 0, true, now)).toBe(false);
  });

  test('nothing goes while sharing is turned off', () => {
    expect(dueToSend({ ...base, autoSend: false }, 3, true, now)).toBe(false);
  });

  test('nothing goes when the browser has not granted the permission', () => {
    expect(dueToSend(base, 3, false, now)).toBe(false);
  });
  ...
  test('a burst of frames reporting at once becomes one send, not five', () => {
    ...
    expect(dueToSend(justSent, 3, true, now)).toBe(false);
  });

  test('the next question after that burst still goes', () => {
    const settled = { ...base, lastSentAt: new Date(now - COALESCE_MS - 1).toISOString() };
    expect(dueToSend(settled, 1, true, now)).toBe(true);
  });

  test('an unreadable timestamp does not wedge it shut forever', () => {
    expect(dueToSend({ ...base, lastSentAt: 'not a date' }, 1, true, now)).toBe(true);
  });

  test('permission is asked of the browser, never read back from our own record', () => {
    const consent = { ...base, lastSentAt: null };
    expect(dueToSend(consent, 1, false, now)).toBe(false);
    expect(dueToSend(consent, 1, true, now)).toBe(true);
  });
});
